...
// Sanitize the user-supplied Client Code into a filesystem-safe prefix.
// Output: uppercase alphanumerics + underscores. Empty/missing → "CLIENT".
function sanitizeClientCode(code) {
  const cleaned = String(code ?? '')
    .replace(/[^\w\s-]/g, '')   // strip filesystem-illegal chars
    .trim()
...
export async function generateOutputs(sessionId, faqs, needsReview) {
  ...
  const prefix = sanitizeClientCode(meta.clientCode);
  ...
  // 3. Write the output files with a client-code filename prefix so
  //    downloads from multiple runs don't collide.
  const outputFiles = {};
  outputFiles.docx = await writeDocx(sessionDir, faqs, needsReview, prefix);
  outputFiles.txt = writeTxt(sessionDir, faqs, needsReview, prefix);
  outputFiles.reviewXlsx = writeReviewXlsx(sessionDir, needsReview, prefix);
  outputFiles.validationCsv = writeValidationCsv(sessionDir, faqs, needsReview, prefix);
...
async function writeDocx(sessionDir, faqs, needsReview, prefix) {
  const children = [];
  ...
  const filePath = path.join(sessionDir, `${prefix}_FAQ_Formatted.docx`);
...
function writeTxt(sessionDir, faqs, needsReview, prefix) {
  const lines = [];
  ...
  const filePath = path.join(sessionDir, `${prefix}_FAQ_Formatted.txt`);
...
function writeReviewXlsx(sessionDir, needsReview, prefix) {
  ...
  const filePath = path.join(sessionDir, `${prefix}_FAQ_Needs_Review.xlsx`);
...
function writeValidationCsv(sessionDir, faqs, needsReview, prefix) {
  const typeLabel = {
    normal: 'Normal',
    pushback: 'Push-back',
    rto: 'RTO',
  ...
  const filePath = path.join(sessionDir, `${prefix}_FAQ_Validation_Report.csv`);
