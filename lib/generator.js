...
export async function generateOutputs(sessionId, faqs, needsReview) {
  ...
  //    We send a single combined list so one batch can cover multiple items.
  const allItems = [
    ...faqs.map((f, i) => ({ bucket: 'faqs', index: i, title: f.title, answer: f.answer })),
    ...needsReview.map((r, i) => ({ bucket: 'needsReview', index: i, title: r.title, answer: r.answer ?? '' })),
  ];
  const categories = await classifyAll(client, allItems);
...
async function classifyBatch(client, chunk) {
  // Primary signal is the title. Only include a snippet when the title is
  // ambiguous (< 5 words) to save tokens.
  const listText = chunk.map((it, i) => {
    const words = it.title.trim().split(/\s+/).filter(Boolean);
    const needsSnippet = words.length < 5 && it.answer;
    const snippet = needsSnippet
      ? ` | Context: ${String(it.answer).slice(0, 150).replace(/\s+/g, ' ').trim()}`
      : '';
...
If nothing fits, return "OTHERS". Return ONLY a JSON array of strings with the same length as the input list, in order. No prose, no markdown.`;
